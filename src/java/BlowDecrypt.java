 ...
 * @author ANAND
 */
public class BlowDecrypt extends HttpServlet {
      ...
      public void doPost(HttpServletRequest request, HttpServletResponse response)
		   ...
		   String strQuery= ""; 
			...
			conn = DriverManager.getConnection(url+dbName,userName,password);
             ...
             strQuery="select * from buploads where fname='"+name+"' ";
					...
					while(rs.next())
                                        {
                                      email=rs.getString("bdata");
					 pwd=rs.getString("bkey");
                                         System.out.println("u2");
                                       // System.out.println(p2);
                                        }
                                       if(country.equalsIgnoreCase("aes")){
                                       ...
                                       RequestDispatcher rd=request.getRequestDispatcher("/BlowDecrypt.jsp");
						rd.forward(request, response);
						System.out.println("ret-forward");
                                       }
                                       else if(country.equalsIgnoreCase("blow")){
                                           System.out.println("BLOW DECRYPT"+email);
...
//				conn.close();
  ...
  byte[] decodedKey = Base64.getDecoder().decode(encodedKey);
    SecretKey originalKey = new SecretKeySpec(decodedKey, 0, decodedKey.length, "DES");
      ...
      decrypted = dec(ptext);
      System.out.println("encrypted DES data"+decrypted);
