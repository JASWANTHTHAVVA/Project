 ...
 * @author ANAND
 */
public class AESDecrypt extends HttpServlet {
      ...
      public void doPost(HttpServletRequest request, HttpServletResponse response)
		   ...
		   String strQuery= ""; 
			...
			conn = DriverManager.getConnection(url+dbName,userName,password);
             ...
             strQuery="select * from uploads where fname='"+name+"' ";
					...
					while(rs.next())
                                        {
                                      email=rs.getString("udata");
					 pwd=rs.getString("ukey");
                                         System.out.println("u2");
                                       // System.out.println(p2);
                                        }
                                       if(country.equalsIgnoreCase("aes")){
                                        AESenc en=new AESenc();
                                        state=en.decrypt(email,user);
                                        
                                        System.out.println(state);
                                        
                                       request.setAttribute("dataa",state);
                                       RequestDispatcher rd=request.getRequestDispatcher("/AESDecrypt.jsp");
						rd.forward(request, response);
						System.out.println("ret-forward");
                                       }
                                      else if(country.equalsIgnoreCase("des")){
