 ...
 * @author ANAND
 */
public class IBEUpload extends HttpServlet {
			...
			conn = DriverManager.getConnection(url+dbName,userName,password);
					...
					user = request.getParameter("fname").toString();
                               ...
                               String keys="thiskeyisUSEDforEncryption";     
                                 strQuery="select * from iberegister where username='"+email+"'";
					...
					while(rs.next())
                                        {
                                      us=rs.getString("ikey");
					 pw=rs.getString("password");
                                         System.out.println("us"+us);
                                       // System.out.println(p2);
                                        }
                               if(name.equalsIgnoreCase("ibe")){
                                   long start = System.nanoTime();
                                       IBEenc en=new IBEenc();
                                       country=en.encrypt(pwd,us);
                           ...
                           strQuery="insert into ibeuploads (fname,users,idata,ikey,algorithm) values('"+user+"','"+email+"','"+country+"','"+us+"','"+name+"')";
                                    System.out.println(strQuery);
                                    st = conn.createStatement();
                                    st.executeUpdate(strQuery);
                        String sql = "UPDATE graph " +
            "SET yaxis = '"+elapsedTimeInSecond+"' WHERE id =2";
         st.executeUpdate(sql);
         RequestDispatcher rd=request.getRequestDispatcher("/ibeupload.jsp");
   ...
   String emp = String.format("%06d", number);
                            long start = System.nanoTime();
                          String desenc = hash(pwd);
                           long stop = System.nanoTime();
                                     long ss = stop - start;
                                double elapsedTimeInSecond = (double) ss / 1_000_000_000;
                                 long convert = TimeUnit.SECONDS.convert(ss, TimeUnit.NANOSECONDS);
                                 long durationInMs = TimeUnit.NANOSECONDS.toMillis(ss);
                           strQuery="insert into ibeuploads (fname,users,idata,ikey,algorithm) values('"+user+"','"+email+"','"+desenc+"','"+emp+"','"+name+"')";
                                    System.out.println(strQuery);
                                    st = conn.createStatement();
                                    st.executeUpdate(strQuery);
                        String sql = "UPDATE graph " +
            "SET xaxis = '"+elapsedTimeInSecond+"' WHERE id =2";
         st.executeUpdate(sql);
         RequestDispatcher rd=request.getRequestDispatcher("/ibeupload.jsp");
						rd.forward(request, response);
						System.out.println("ret-forward");
                          System.out.println("HASH enc:"+desenc);
                       }
                               
                               else{
                                  out.println("<script type=\"text/javascript\">");
                        out.println("alert('SELECT RIGTH ALGORITHM');");
                     out.println("location='ibeupload.jsp';");
            out.println("</script>");		 
...
//				conn.close();
  ...
  String encrypted;
  private String hash(String ptext) throws NoSuchPaddingException,
  InvalidKeyException {
    long start = System.nanoTime();
    try {
